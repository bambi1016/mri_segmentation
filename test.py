...
test_dir = join(args.data_dir)
batch_size = 1


def inference(target_dir):
    volume_size = args.size*3 if len(args.size) == 1 else args.size
    test_loader = DataLoader(MRIDataset(target_dir,
                                       volume_size,
                                       sampling_mode='center',
                                       deterministic=True),
                            batch_size=1,
                            num_workers=0)
    input_paths = test_loader.dataset.inputs
    label_paths = test_loader.dataset.labels
    ...
    for i, (inputs, labels) in enumerate(test_loader):
        ...
        full_labels = test_loader.dataset._load_full_label(label_paths[i])
        full_preds = test_loader.dataset._project_full_label(input_paths[i],
                                                            preds.cpu())
        preds = full_preds
        labels = full_labels
    ...
    print("Micro Averaged Dice {}, {}".format(sum_hd / len(test_loader),
                                              sum_sd / len(test_loader)))
